...
export const useAuth = ({ middleware, redirectIfAuthenticated }: IUseAuth) => {
    ...
    const { data: user, error, mutate } = useSWR<User>('/api/collections/users/records', () =>
        axios
    ...
    const register = async (args: IApiRequest) => {
        ...
        axios
            .post('/api/collections/users/records', props)
            .then(() => mutate())
            .catch(error => {
    ...
    const login = async (args: IApiRequest) => {
        ...
        // axios
        //     .post('/api/collections/users/auth-with-password', props)
        //     .then(() => mutate())
        //     .catch(error => {
        //         if (error.response.status !== 422) throw error
        //         setErrors(error.response.data.errors)
        ...
        await pb.collection('users').authWithPassword(props.email, props.password);
    }
